...
function getConnection() {
  const url = process.env.PLANETSCALE_URL;

  if (!url) {
    throw new Error('Missing PLANETSCALE_URL environment variable');
  }

  return connect({ url });
}

type PlanetScaleConnection = ReturnType<typeof getConnection>;
